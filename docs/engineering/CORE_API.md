...
`shardIndexForCoordinate` implements the fixed zero-based assignment `uint64be(SHA256(UTF8(coordinateId))[0..8]) mod M`; `assignedShardCoordinateIds` adapts it to one-based CLI shards. `parseShardSpecifier` accepts exact `N/M`. `parseShardPlan` and `parseShardBundleManifest` require exact RFC 8785 source bytes and enforce canonical order, bounded coordinate/path counts, digests, timestamps, complete assigned/executed identity, report-schema/retention agreement, retained-capture counts, and privacy-safe mask ID/cardinality ordering. Bundle manifest v2 adds only capture totals and mask metadata needed to reproduce the final evidence manifest; it contains no selectors or DOM/captured values. Invalid data raises `ShardValidationError`. These APIs perform no filesystem or browser work; the runner consumes them for aggregation.
