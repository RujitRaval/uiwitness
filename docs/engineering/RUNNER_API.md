...
Shard publication is deliberately separate from complete generation persistence. It exclusively creates `.uiwitness/shards/<runSetId>/<n>-of-<m>/`, stores retained PNG bytes under `evidence/artifacts/`, writes `report.json`, and records every byte count, SHA-256 digest, retention total, and privacy-safe mask ID/cardinality aggregate in canonical manifest v2. The manifest is written and fsynced last as the bundle commit marker. A path collision is `SHARD_BUNDLE_EXISTS`; unsafe boundaries and publication failures reject without overwriting. Shards do not acquire `.runner-persistence-lock`, update `.uiwitness/generation.json`, or mutate `.uiwitness/report/`.

`mergeShardScenarioBundles(options)` is the sole runner API that promotes sharded evidence into a complete generation. It accepts explicit bundle roots plus the current full matrix, base URL, config/contract digests, evidence policy, tool version, and live merge clock. Input count is checked before filesystem work; manifests are loaded sequentially within a 16 MiB aggregate limit, and declared report/evidence content is capped at 256 MiB before any content read. Header, completeness, reconstructed-plan, current-binding, path-collision, exact-tree, size/checksum, report, coordinate-metadata, screenshot, and privacy-summary validation all complete before it acquires the final publication lock and rechecks plan activity. Executions and bytes normalize into current deterministic configuration order so the final report matches the unsharded path, and `finalizeGeneration(report, plan)` supplies the CLI-owned comparison/verdict/proposal sidecars while the existing crash-recoverable transaction publishes final truth. Invalid inputs raise `ShardBundleError` and leave the prior generation untouched.

## Current boundary

Phase 3 and State Contract Guard T5/T9/T10/T11/T12 are complete. The runner owns browser reuse, per-cell isolation, once-per-run memory-only authentication, scenarios/hooks, viewport/theme, navigation/readiness, fail-closed masked screenshot capture, retention, sanitized diagnostics, assertions, failure policies, core result translation, crash-recoverable generation persistence, isolated immutable shard bundles, and fail-closed aggregate publication. The CLI consumes this API through `scan`, `check`, and `guard`; the report package remains a pure validated v1/v2 renderer, and contract comparison/shard contracts remain core-owned.
