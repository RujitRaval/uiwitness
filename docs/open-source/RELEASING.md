...
The completed State Contract Guard release proof is [`v0.26.13`](https://github.com/RujitRaval/uiwitness/releases/tag/v0.26.13) from commit `64c6f6dd0f541a5f79c1ec165080ed9e5a8a316b`. Protected [release run `34155694765`](https://github.com/RujitRaval/uiwitness/actions/runs/34155694765) prepared and exercised the exact four-package artifact set on Node 24 and Node 22 before publishing `uiwitness-core`, `uiwitness-report`, `uiwitness-runner-playwright`, and `uiwitness` at `0.26.13` through trusted-publishing OIDC; token configuration and bootstrap cleanup were skipped. npm provenance verification bound every package and integrity digest to `.github/workflows/release.yml`, `refs/tags/v0.26.13`, and that exact commit. Both automatic registry-only jobs then passed the complete check → promotion → scan → open journey with all eight matrix cells. This is the live end-to-end proof for T14; `v0.25.5` remains the first normal token-free release proof and `v0.25.4` remains the bootstrap record.
