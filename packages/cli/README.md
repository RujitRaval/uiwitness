...
npx uiwitness scan
npx uiwitness guard
npx uiwitness guard shard-plan --shards 4 --out .uiwitness/shard-plan.json
npx uiwitness guard --shard 1/4 --shard-plan .uiwitness/shard-plan.json
npx uiwitness guard merge --input .uiwitness/shards/<run-set-id>/1-of-4 --input .uiwitness/shards/<run-set-id>/2-of-4 --input .uiwitness/shards/<run-set-id>/3-of-4 --input .uiwitness/shards/<run-set-id>/4-of-4
...
`guard shard-plan` snapshots the complete current config inventory into a canonical, random-nonce-bound, expiring plan. Every worker receives that exact file and runs one `--shard N/M`. Shards use fixed SHA-256 assignment and publish immutable checksummed bundles under `.uiwitness/shards/<runSetId>/<n>-of-<m>/` without changing the latest report or issuing a partial verdict. Bundle completion exits `0` even when failed cells were recorded. `guard merge` accepts one repeated `--input` per bundle, rejects any incomplete, mixed, expired, drifted, linked, or checksum-invalid set, then compares once and atomically publishes the final report, verdict, and optional proposal generation. Plans, shards, and merge reject authenticated configs.
...
The package also exports `defineConfig`, `checkPublicSite`, config discovery/loading, initialization, scan orchestration, shard-plan/shard/merge orchestration, and report-opening APIs for TypeScript consumers. Generated public-site scenarios import the narrow `uiwitness/public-site-scenario` helper so they retain the same assertions as Quick Check without copying implementation code.
